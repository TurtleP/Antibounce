		...
		if name ~= "tile" then --not updating tiles!
			for _, objData in pairs(objectT) do --check object variables
				if objData.active and not objData.static then
					...
					if gravity == 1 then
						objData.speedy = math.min(objData.speedy + objData.gravity * dt, 30 * 8) --add gravity to objects
					else
						objData.speedy = math.max(objData.speedy + objData.gravity * dt, -(30 * 8)) --add gravity to objects
					end
